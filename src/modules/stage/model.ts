//定义本模块的业务模型
import {BaseModel, ErrorCodes, LoadingState, effect, reducer} from '@elux/vue-web';
...
export class Model extends BaseModel<ModuleState, APPState> {
  ...
  protected getRouteParams(): RouteParams {
    ...
    console.warn(`${subModuleStr} | ${curViewStr}`);
  ...
  @reducer
  protected putCurUser(curUser: CurUser): void {
    this.state.curUser = curUser;
  }

  //定义一个effect，用来执行登录逻辑  effect ==> 触发reducer
