...
> {
  ...
  @reducer
  public putList(listSearch: TDefineResource['ListSearch'], items: TDefineResource['ListItems'][], pagination: TDefineResource['Pagination']): void {
    Object.assign(this.state, {listSearch, list: items, pagination});
  }
  ...
  public async fetchList(listSearchData?: TDefineResource['ListSearch']): Promise<void> {
    ...
    console.log('初始化列表..........>' + JSON.stringify(listSearch));
    const {items, pagination} = await this.api.getList!(listSearch);
    this.dispatch(this.privateActions.putList(listSearch, items, pagination));
  }
...
export function useSearch<TFormData>(props: {listPathname: string}, defaultListSearch: Partial<TFormData>) {
  ...
  console.log(`搜索-------->${props.listPathname}`)
  const onSearch = (values: Partial<TFormData>) => {
    const searchQuery = excludeDefaultParams(defaultListSearch, {...values, pageIndex: 1});
...
export function useTableChange<T extends BaseListSearch>(props: {listSearch?: T}, listPathname: string, defaultListSearch: T) {
  ...
  return (pagination: any, filter: any, _sorter: any) => {
    ...
    const {pageIndex, pageSize} = pagination as {pageIndex: number; pageSize: number};
    ...
    // const pageIndex = currentSorter !== sorterStr ? 1 : current;
    console.log(JSON.stringify(router.location));
    const searchQuery = excludeDefaultParams(defaultListSearch, {...listSearch, pageIndex, pageSize});
    console.log(`useTableChange---invoke>>>>` + JSON.stringify(searchQuery));
